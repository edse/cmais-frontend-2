  ...
  if($usuario){
    ...
    if($usuario->valido == "true"){
      ...
      */
      ...
      $script .= '$("#email22").val(\''.$_REQUEST["f1_email"].'\');$("#f4_email2").val(\''.$_REQUEST["f1_email"].'\');$("#f4_email").val(\''.$_REQUEST["f1_email"].'\');$("#f4_nome").val("'.$usuario->nome.'");$("#f4_endereco").val(\''.$usuario->endereco.'\');$("#f4_numero").val(\''.$usuario->numero.'\');$("#f4_complemento").val(\''.$usuario->complemento.'\');$("#f4_cep").val(\''.$usuario->cep.'\');$("#f4_bairro").val(\''.$usuario->bairro.'\');$("#f4_telefone").val(\'('.$usuario->ddd.') '.$usuario->telefone.'\');$("#f4_twitter").val(\''.$usuario->twitter.'\');$("#f4_cod_faixaetaria").append(\''.$idade_options.'\');$("#f4_cod_sexo").append(\''.$sexo_options.'\');$("#f4_localexterior").val("'.$usuario->localexterior.'");$("#f4_cod_recepcaodesinal").append(\''.$recepcao_options.'\');$("#f4_estado").append(\''.$uf_options.'\');$("#f4_local").append(\''.$municipio_options.'\');$("#f4_sms").attr(\'checked\', '.$usuario->sms.');$("#f4_newsletter").attr(\'checked\', '.$usuario->newsletter.');$("#f4_convite").attr(\'checked\', '.$usuario->convite.');$("#f4_terceiros").attr(\'checked\', '.$usuario->terceiros.');$("#f4_email2").val(\''.$_REQUEST["f1_email"].'\');$("#f4_email").val(\''.$_REQUEST["f1_email"].'\');$("#row1").hide();$("#f4_cod_veiculo").children().remove();$("#f4_cod_veiculo").append(\''.$veiculo_options.'\');$("#f4_cod_programa").find("option").attr("value", "validacao");$("#f4_mensagem").val("");toggleExterior();$("f1_email").val("");$("select").each(function(i){$("option").each(function(j){/*console.log($(this).text())*/if($(this).text()=="SIC")$(this).remove()});});';
  ...
  */
  ...
  if(isset($usuario->codigo) && $usuario->codigo > 0 && $usuario->valido == "true"){
    ...
    if($result->altera_pessoaResult > 0){
      ...
      $script = '$("#f4_mais").attr("checked","false");$("#row1, #row2, #row3, #row4, #row5, #row6, #row7, #row8").hide();$("#row9").show();$("#f4_cod_programa").find("option").attr("value", "--");';
