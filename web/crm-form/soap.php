...
if(($_REQUEST["step"]==1)&&($_REQUEST["f1_email"]!="")){
  ...
  */
  ...
  if(isset($usuario->codigo) && $usuario->codigo > 0 && $usuario->valido == "true"){
    ...
    if($result->altera_pessoaResult > 0){
      $script = '$("#row1, #row2, #row3, #row9, #row5, #row6, #row7, #row8").hide();$("#row4, #message, .control-group.f4_mais, #row9").show();$("#f4_maisinfo, #btn5").hide();("#f4_mais").attr("checked", "false");$("#row9").delay(3000).slideUp("fast"); ';
