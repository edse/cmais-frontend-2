...
if(($_REQUEST["step"]==1)&&($_REQUEST["f1_email"]!="")){
  ...
  */
  ...
  if(isset($usuario->codigo) && $usuario->codigo > 0 && $usuario->valido == "true"){
    ...
    if($result->altera_pessoaResult > 0){
      $script = '$(".control-group.f4").removeClass("error");$(".f4_cod_programa").removeAttr("selected");$(".salvar-alteracoes").hide();$("#f4_cod_veiculo, #f4_cod_assunto, #f4_mensagem").removeAttr("disabled");$("#row4, #message, .control-group.f4_mais, #row9").show();$("#f4_maisinfo, #btn5").hide();set_checked(false);$("#row9").delay(3000).slideUp("fast");$("#row4").find(".success").each(function(){$(this).removeClass("success")});$("#row4").find("label.valid").each(function(){$(this).remove()});';
