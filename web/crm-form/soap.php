...
if(($_REQUEST["step"]==1)&&($_REQUEST["f1_email"]!="")){
  ...
  if($usuario){
    ...
    if($usuario->valido == "true"){
      ...
      */
      ...
      $script .= '$("#email22").val(\''.$_REQUEST["f1_email"].'\');$("#f4_email2").val(\''.$_REQUEST["f1_email"].'\');$("#f4_email").val(\''.$_REQUEST["f1_email"].'\');$("#f4_nome").val("'.$usuario->nome.'");$("#f4_endereco").val(\''.$usuario->endereco.'\');$("#f4_numero").val(\''.$usuario->numero.'\');$("#f4_complemento").val(\''.$usuario->complemento.'\');$("#f4_cep").val(\''.$usuario->cep.'\');$("#f4_bairro").val(\''.$usuario->bairro.'\');$("#f4_telefone").val(\'('.$usuario->ddd.') '.$usuario->telefone.'\');$("#f4_twitter").val(\''.$usuario->twitter.'\');$("#f4_cod_faixaetaria").append(\''.$idade_options.'\');$("#f4_cod_sexo").append(\''.$sexo_options.'\');$("#f4_localexterior").val("'.$usuario->localexterior.'");$("#f4_cod_recepcaodesinal").append(\''.$recepcao_options.'\');$("#f4_estado").append(\''.$uf_options.'\');$("#f4_local").append(\''.$municipio_options.'\');$("#f4_sms").attr(\'checked\', '.$usuario->sms.');$("#f4_newsletter").attr(\'checked\', '.$usuario->newsletter.');$("#f4_convite").attr(\'checked\', '.$usuario->convite.');$("#f4_terceiros").attr(\'checked\', '.$usuario->terceiros.');$("#f4_email2").val(\''.$_REQUEST["f1_email"].'\');$("#f4_email").val(\''.$_REQUEST["f1_email"].'\');$("#row1").hide();$("#row4").show();$("#f4_cod_veiculo").children().remove();$("#f4_cod_veiculo").append(\''.$veiculo_options.'\');$("#f4_mensagem").val("");toggleExterior();$("f1_email").val("");';
  ...
  */
  ...
  if(isset($usuario->codigo) && $usuario->codigo > 0 && $usuario->valido == "true"){
    ...
    if($result->altera_pessoaResult > 0){
      $script = '$("#f4_cod_veiculo, #f4_cod_assunto, #f4_mensagem").removeAttr("disabled");$("#row4, #message, .control-group.f4_mais, #row9").show();$("#f4_maisinfo, #btn5").hide();set_checked(false);$("#row9").delay(3000).slideUp("fast");$("#row4").find(".success").each(function(){$(this).removeClass("success")});$("#row4").find(".valid").each(function(){$(this).removeClass("valid")});';
