...
class Connection {
  ...
  private function handshake($data) {
    ...
    if (preg_match("/policy-file-request/i", $data) || preg_match("/crossdomain/i", $data)) {
      $this->log("[" . date('Y-m-d H:i:s') . "] CROSSDOMAIN.XML REQUEST\n");
      $contents = '<?xml version="1.0"?><cross-domain-policy><allow-access-from domain="*" to-ports="*" /></cross-domain-policy>';
      $this->server->writeBuffer($this->socket, $contents);
    ...
    /*
    if ($this->server->getApplication('quiz') !== false) {
      $this->server->getApplication('quiz')->clientConnected($this);
    }*/
  ...
  public function setTime($p) {
    $this->time = $p;
  }

  public function setToken($p) {
    $this->token = $p;
  }

  public function setAnswered($p) {
    $this->answered = $p;
  }

  public function setAvatar($p) {
    $this->avatar = $p;
  }
