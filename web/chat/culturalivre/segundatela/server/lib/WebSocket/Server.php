...
class Server extends Socket {
  ...
  private $_maxConnectionsPerIp = 1000;
  private $_maxRequestsPerMinute = 1200;
  private $_host = null;
  private $_port = 0;
          ...
          if (($ressource = stream_socket_accept($this->master)) === false) {
            ...
            if (count($this->clients) > $this->_maxClients) {
              ...
              if ($this->getApplication('quiz') !== false) {
                echo "\nAttention: Client Limit Reached!";
                //$this->getApplication('quiz')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              if ($this->getApplication('secondscreenapp') !== false) {
                echo "\nAttention: Client Limit Reached!";
                //$this->getApplication('secondscreenapp')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              if ($this->getApplication('secondscreenjornaldacultura') !== false) {
                echo "\nAttention: Client Limit Reached!";
                //$this->getApplication('secondscreenjornaldacultura')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              if ($this->getApplication('secondscreencartaoverde') !== false) {
                echo "\nAttention: Client Limit Reached!";
                //$this->getApplication('secondscreencartaoverde')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              if ($this->getApplication('secondscreenrodaviva') !== false) {
                echo "\nAttention: Client Limit Reached!";
                //$this->getApplication('secondscreenrodaviva')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              if ($this->getApplication('secondscreenqss') !== false) {
                echo "\nAttention: Client Limit Reached!\n";
                //$this->getApplication('secondscreenqss')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              */
              continue;
            }
            ...
            if ($this->_checkMaxConnectionsPerIp($client->getClientIp()) === false) {
              ...
              if ($this->getApplication('quiz') !== false) {
                echo "\nConnection/Ip limit for ip ".$client->getClientIp()." was reached!";
                //$this->getApplication('quiz')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              if ($this->getApplication('secondscreenapp') !== false) {
                echo "\nConnection/Ip limit for ip ".$client->getClientIp()." was reached!";
                //$this->getApplication('secondscreenapp')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              if ($this->getApplication('secondscreenjornaldacultura') !== false) {
                echo "\nConnection/Ip limit for ip ".$client->getClientIp()." was reached!";
                //$this->getApplication('secondscreenjornaldacultura')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              if ($this->getApplication('secondscreencartaoverde') !== false) {
                echo "\nConnection/Ip limit for ip ".$client->getClientIp()." was reached!";
                //$this->getApplication('secondscreencartaoverde')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              if ($this->getApplication('secondscreenrodaviva') !== false) {
                echo "\nConnection/Ip limit for ip ".$client->getClientIp()." was reached!";
                //$this->getApplication('secondscreenrodaviva')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              if ($this->getApplication('secondscreenqss') !== false) {
                echo "\n'Connection/Ip limit for ip ".$client->getClientIp()." was reached!\n";
                //$this->getApplication('secondscreenqss')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              */
              continue;
            }
  ...
  public function registerApplication($key, $application) {
    ...
    $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
    $this->applications[$key]->setServerInfo($serverInfo);
    $this->applications[$key]->socket_id = $this->socket_id;

    if ($key === 'chatapp') {
      $this->applications[$key]->setContents(json_decode(file_get_contents("log/chat.json")));
      echo "\n".file_get_contents("log/chat.json");
    }

    /*
    // status is kind of a system-app, needs some special cases:
    if ($key === 'status') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, );
      $this->applications[$key]->setServerInfo($serverInfo);
    }
    if ($key === 'quiz') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
    ...
    if ($key === 'secondscreenapp') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
      $this->applications[$key]->setServerInfo($serverInfo);
    }
    if ($key === 'secondscreenjornaldacultura') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
      $this->applications[$key]->setServerInfo($serverInfo);
    }
    if ($key === 'secondscreencartaoverde') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
      $this->applications[$key]->setServerInfo($serverInfo);
    }
    if ($key === 'secondscreenrodaviva') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
      $this->applications[$key]->setServerInfo($serverInfo);
    }
    if ($key === 'secondscreenqss') {
      $serverInfo = array('maxClients' => $this->_maxClients, 'maxConnectionsPerIp' => $this->_maxConnectionsPerIp, 'maxRequetsPerMinute' => $this->_maxRequestsPerMinute, 'host' => $this->_host, 'port' => $this->_port );
      $this->applications[$key]->setServerInfo($serverInfo);
    }
    $this->applications[$key]->socket_id = $this->socket_id;
  ...
  private function _checkRequestLimit($clientId) {
    ...
    if (!isset($this->_requestStorage[$clientId])) {
      $this->_requestStorage[$clientId] = array('lastRequest' => time(), 'totalRequests' => 1);
      return true;
    }

    if($this->getApplication('secondscreenqss') !== false) {
      if($this->getApplication('secondscreenqss')->getGodofredo() == $clientId)
        return true;
    }
  ...
  public function setMaxConnectionsPerIp($limit) {
    ...
    //AQUI
    $this->_maxConnectionsPerIp = 1000;
    return true;
  }
  ...
  public function setMaxRequestsPerMinute($limit) {
    ...
    //AQUI
    $this->_maxRequestsPerMinute = 1200;
    return true;
  }
  ...
  public function keepAlive() {
    $now = time();
    if($this->_lastKeepAliveTime + $this->_keepAliveInterval < $now) {
      echo "\n\n ---- Keep alive (".date("H:i:s").") ---- \n\n";
      $this->_lastKeepAliveTime = $now;
      if(count($this->clients)>0) {
        /*
        if($this->getApplication('secondscreenqss') !== false) {
          echo "\n\n ---- Saving Ranking ---- \n\n";
          $this->getApplication('secondscreenqss')->_saveRanking();
        }
        */
        foreach($this->clients as $client) {
          if($this->getApplication('secondscreenqss') !== false) {
            $this->getApplication('secondscreenqss')->_ping($client);
          }
          else{
