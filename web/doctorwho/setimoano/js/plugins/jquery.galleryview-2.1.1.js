...
*/
	...
	*/
		function buildFilmstrip() {
			...
			var pointer = $('<div></div>');
			pointer.addClass('pointer').addClass('png_bg').appendTo(j_gallery).css({
				 'position':'absolute',
				 'zIndex':'1000',
				 'width':'0px',
				 'fontSize':'0px',
			...
			var navNext = $('<img />');
			navNext.addClass('nav-next').addClass('png_img').attr('src',theme_path+opts.nav_theme+'/next.png').appendTo(j_gallery).css({
				'position':'absolute',
				'cursor':'pointer'
			}).click(showNextItem);
			var navPrev = $('<img />');
			navPrev.addClass('nav-prev').addClass('png_img').attr('src',theme_path+opts.nav_theme+'/prev.png').appendTo(j_gallery).css({
				'position':'absolute',
				'cursor':'pointer'
			}).click(showPrevItem);
			if(filmstrip_orientation=='horizontal') {
