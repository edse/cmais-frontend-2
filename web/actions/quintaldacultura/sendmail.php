    ...
    if ($current_time < $expiration_time) {
      ...
      if($_FILES['datafile']['size'] <= 0) {
        ...
        if(mail($to, $subject, $message, $headers)) {
          //die("1");
          header("Location: http://cmais.com.br/quintaldacultura?success=1");
        }else{
          //die("2");
          header("Location: http://cmais.com.br/quintaldacultura?error=2");
        }
      }
      ...
      if (!in_array($file_mime_type, array("image/gif", "image/png", "image/jpg"))) {
        ...
        if (unlink($_FILES['datafile']['tmp_name'])) {
          header("Location: http://cmais.com.br/quintaldacultura?error=3");
          die();
        }
      }
      else {
        ...
        if(sendMailAtt($to, $from, $subject, $message, $attach)) {
          if (unlink($_FILES['datafile']['tmp_name'])) {
            header("Location: http://cmais.com.br/quintaldacultura?success=2");
            die("3");
          }
        }
        else{
          if (unlink($_FILES['datafile']['tmp_name'])) {
            header("Location: http://cmais.com.br/quintaldacultura?error=1");
            die();
          }
        }
      }
      ...
      if (unlink($_FILES['datafile']['tmp_name'])) {
        header("Location: http://cmais.com.br/quintaldacultura?error=4");
        die();
      }
    }
  }
