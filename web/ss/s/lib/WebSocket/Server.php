...
class Server extends Socket {
  ...
  private $_maxClients = 2000;
  private $_maxConnectionsPerIp = 200;
  private $_maxRequestsPerMinute = 60;
  private $_host = null;
  private $_port = 0;
        ...
        if ($socket == $this->master) {
          if (($ressource = stream_socket_accept($this->master)) === false) {
            ...
            if (count($this->clients) > $this->_maxClients) {
              ...
              if ($this->getApplication('secondscreen') !== false) {
                $this->getApplication('secondscreen')->statusMsg('Attention: Client Limit Reached!', 'warning');
              }
              continue;
            }
            ...
            if ($this->_checkMaxConnectionsPerIp($client->getClientIp()) === false) {
              ...
              if ($this->getApplication('secondscreen') !== false) {
                $this->getApplication('secondscreen')->statusMsg('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!', 'warning');
              }
              continue;
            }
          }
  ...
  public function removeClientOnClose($client) {
    ...
    if ($this->getApplication('secondscreen') !== false) {
      $this->getApplication('secondscreen')->clientDisconnected($clientIp, $clientPort);
    }
