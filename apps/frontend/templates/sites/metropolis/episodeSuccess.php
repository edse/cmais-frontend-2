          		...
          		<div class="centroRV">
          			<div class="video-interna">
						...
						<span class="faixa" style="height:5px; margin-top:5px;"></span>
		                  <h3>
		                  	<?php
		                  		if (preg_match("/^[0-9]{8}$/",trim($asset->getTitle())))
		                  		{
		                  			$year = substr($asset->getTitle(),0,4);
		                  			$month = substr($asset->getTitle(),4,2);
		                  			$day = substr($asset->getTitle(),6,2);
														echo "Programa exibido em " . $day . "/" . $month . "/" . $year;
													}
													elseif (preg_match("/^[0-9]{8}(_|-){1}(.)*$/",trim($asset->getTitle())))
		                  		{
		                  			...
		                  			$day = substr($asset->getTitle(),6,2);
													...
													elseif (preg_match("/^[0-9-]+$/",trim($asset->getTitle())))
													{
														$aux = explode('-', trim($asset->getTitle()));
														if (count($aux) >= 3) { 
															echo "Programa exibido em " . $aux[2] . "/" . $aux[1] . "/" . $aux[0];
														}
														else
															echo $asset->getTitle();
													}
		                  ...
		                  <?php if (!preg_match("/^[0-9]{8}$/",trim($asset->getTitle()))):?>
