...
<script>
  ...
  function vilaSesamoGetContents() {
    $.ajax({
      ...
      data: "page="+contentPage+"&items=3&site=<?php echo (int)$site->id ?>&section=<?php echo $section->getId(); ?>",
      beforeSend: function(){
      ...
      success: function(data){
        ...
        if (data != "") {
          ...
          $('#container').isotope({filter:'.element'}).append($newEls).isotope('appended',$newEls);//.isotope({ sortBy:'original-order'});
          contentPage++;
        }else{
