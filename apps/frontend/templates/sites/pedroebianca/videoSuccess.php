<?php
if (!isset($asset)) {
  $asset = Doctrine_Query::create() -> select('a.*') -> from('Asset a, AssetVideo av') -> where('a.id = av.asset_id') -> andWhere('a.site_id = ?', (int)$site -> id) -> andWhere('a.asset_type_id = 6') -> andWhere("av.youtube_id != ''") -> andWhere("(a.date_start IS NULL OR a.date_start <= CURRENT_TIMESTAMP)") -> orderBy('a.id desc') -> limit(1) -> fetchOne();
}

$episode = Doctrine_Query::create() -> select('a.*') -> from('Asset a, RelatedAsset r') -> where('r.asset_id = ?', (int)$asset -> id) -> andWhere('a.site_id = ?', (int)$site -> id) -> andWhere('r.parent_asset_id = a.id') -> andWhere('a.asset_type_id = 15') -> andWhere("(a.date_start IS NULL OR a.date_start <= CURRENT_TIMESTAMP)") -> orderBy('a.id desc') -> limit(1) -> fetchOne();
              ...
              <div class="video-interna">
                ...
                <div class="boxVideo">
                  <div class="boxVideoWrapper">
                    ...
                    ?>
                    <a class="" href="<?php echo $asset->retriveUrl() ?>" title="<?php echo $asset->getTitle() ?>"> <img src="<?php echo $asset->retriveImageUrlByImageUsage('image-6') ?>" alt="<?php echo $asset->getTitle() ?>" name="<?php echo $asset->getTitle() ?>" /> <?php elseif($asset->AssetType->getSlug() == "content" || $asset->AssetType->getSlug() == "image-gallery"):
                    ?>
                    ...
                    <div style="width:640px; height:384px;">
                      ...
                      ?></h2><h4><?php echo $asset->getDescription()
                      ?></h4>
                    </div>
                    <?php endif;
                    ?>
                  ...
                  ?>
                  <div class="aceoDestaque vr2">
                    <h3>Programa</h3>
                    <div class="carrossel">
                      <ul>
                        <?php foreach($blocos as $k=>$d):
                        ?>
                        <li><?php if($d->retriveImageUrlByImageUsage("image-2") != ""):
                        ?>
                        <a class="aImg<?php if($d->getId() == (int)$asset->id): ?> ativo<?php endif; ?>" href="<?php echo $d->retriveUrl() ?>"> <img src="<?php echo $d->retriveImageUrlByImageUsage("image-2") ?>" alt="<?php echo $d->getTitle() ?>" /> <span class="ico"></span> </a><?php endif;
                        ?>
                        <?php //if($d->retriveLabel() != ""):
                        ?>
                        <a class="aTxt" href="<?php echo $d->retriveUrl() ?>"> <span class="nomeRlacionado"><?php?></span> <?php /* <span class="nomeTxt"><?php echo $d->getDescription() ?></span> */?> </a><?php //endif;?></li>
                        <?php endforeach;?>
                      </ul>
                    </div>
                  </div>
                  ...
                  <?php if ($episode):
                  ?>
                  <p class="dataPost">Programa exibido em <?php echo format_date($episode->AssetEpisode->getDateRelease(),'D')
                  ?></p>
                  <?php else:?>
                  <p class="dataPost">Programa exibido em <?php echo format_date($asset->getCreatedAt(),'D')
                  ?></p>
                  <?php endif;?>
                  <?php if(($asset->AssetVideo->getHeadline() != "")&&($asset->AssetVideo->getHeadline() != $asset->getTitle())):
                  ?>
                  <p style="font-size: 10px;"><?php echo $asset->AssetVideo->getHeadline()
                  ?></p>
                  <?php endif;?>
                  <p class="post"><?php echo $asset->getDescription()
                  ?></p>
                  <?php endif;?>
                  <?php if ($episode):
                  ?>
                  <?php if ($episode->AssetEpisode->getTranscription()):
                  ?>
                  <span class="faixa" style="height:5px; margin-top:5px;"></span>
                  <div class="transcricao">
                    <h3>Transcrição</h3>
                    <div class="transcricaoWrapper">
                      <?php echo html_entity_decode($episode->AssetEpisode->getTranscription())
                      ?>
                    </div>
                  </div>
                  <?php endif;?>
              ...
              <div class="veja">
                ...
                <div class="vejaTambem">
                  <ul>
                    ...
                    <?php if(($k > 0) && ($k % 3 == 0)):
                    ?>
                    </li><li><?php endif;?><li><?php if($d->retriveImageUrlByImageUsage('image-7')):
                    ?>
                    <a class="aImg<?php if($d->getId() == (int)$asset->id): ?> ativo<?php endif;?>" href="<?php echo $d->retriveUrl() ?>"> <img src="<?php echo $d->retriveImageUrlByImageUsage('image-7') ?>" alt="<?php echo $d->getTitle() ?>" name="<?php echo $d->getTitle() ?>" /> <span class="ico"></span> </a><?php endif;?>
                    <a class="aTxt" href="<?php echo $d->retriveUrl() ?>"> <span class="nomeRlacionado"><?php echo $d->getTitle()
                    ?></span> <!-- span class="nomeTxt"><?php echo $d->getDescription() ?></span--> </a></li>
                    <?php endforeach;?>
                  </ul>
                  <!--
                  <a href="" class="sugestoes"><span>carregar mais sugest&otilde;es</span></a>
                  <div class="publicidade">
                  <img title="publicidade" alt="publicidade" src="../pedroebianca/images/lixo-img.jpg">
                  </div>
                  -->
                </div>
                <?php endif;?>
                  ...
                  <div class="box-charges">
                    <div id="gallery">
                      <ul>
                        ...
                        ?>
                        <li><a class="charges_caruso" rel="charges_caruso" href="<?php echo $d->retriveImageUrlByImageUsage("image-6-b") ?>" title="<?php echo $d->getTitle() ?>"> <img alt="<?php echo $d->getTitle() ?>" src="<?php echo $d->retriveImageUrlByImageUsage("image-1-b") ?>"> </a></li>
                        <?php endforeach;?>
                      </ul>
                    </div>
                  </div>
