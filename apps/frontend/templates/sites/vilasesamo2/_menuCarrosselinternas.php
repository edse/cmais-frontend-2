 <div id="carrossel-interna">
    <div class="carrossel-i" id="carrossel-i"> 
      <div class="slider">
          <div class="slider-mask-wrap">
            <div class="slider-mask">
              <ul class="slider-target">
                <li class="video">
                  <div class="inner personagens bel">
                    <a href="/vilasesamo2jogos/nomedojogo1" title="Nome do jogo 1" class="btn-bel">
                     <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                     </a>
                  </div>
                  <a class="nome" href="/vilasesamo2/jogos/nomedojogo1" title="Nome do Jogo 1"><i class="sprite-ico-atividades-p"></i>Nome do Jogo 1</a>
                </li>
                <li class="jogo">
                  <div class="inner personagens beto">
                    <a href="/vilasesamo2/jogos/nomedojogo2" title="Nome do jogo 2" class="">
                     <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                  </div>
                  <a class="nome" href="/vilasesamo2/jogos/nomedojogo2" title="Nome do Jogo 2"><i class="sprite-ico-videos-p">Nome do Jogo 2</a>
                </li>
                <li class="atividade">
                  <div class="inner personagens comecome">
                    <a href="/vilasesamo2/jogos/nomedojogo3" title="Nome do jogo 3" class="">
                      <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                  </div>
                  <a class="nome" href="/vilasesamo2/jogos/nomedojogo3" title="Nome do Jogo 3"><i class="sprite-ico-jogos-p">Nome do Jogo 3</a>
                </li>
                <li>
                  <div class="inner personagens elmo">
                    <a href="/vilasesamo2/jogos/nomedojogo4" title="Nome do jogo 4" class="">
                      <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                  </div>
                  <a class="nome" href="/vilasesamo2/jogos/nomedojogo4" title="Nome do Jogo 4"><i class="sprite-ico-atividades-p">Nome do Jogo 4</a>
                </li>
                <li>
                  <div class="inner personagens enio">
                    <a href="/vilasesamo2/jogos/nomedojogo5" title="Nome do Jogo 5" class="">
                      <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                   </div>
                   <a class="nome" href="/vilasesamo2/jogos/nomedojogo5" title="Nome do Jogo 5"><i class="sprite-ico-atividades-p">Nome do Jogo 5</a>
                </li>
                <li>
                  <div class="inner personagens garibaldo">
                    <a href="/vilasesamo2/jogos/nomedojogo6" title="Nome do Jogo 6" class="">
                      <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                  </div>
                  <a class="nome" href="/vilasesamo2/personagens/garibaldo" title="Nome do Jogo 6"><i class="sprite-ico-atividades-p">Nome do Jogo 6</a>
                </li>
                <li>
                  <div class="inner personagens grover">
                    <a href="/vilasesamo2/jogos/nomedojogo7" title="Nome do Jogo 7" class="">
                      <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                  </div>
                  <a class="nome" href="/vilasesamo2/jogos/nomedojogo7" title="Nome do Jogo 7"><i class="sprite-ico-atividades-p">Nome do Jogo 7</a>
                </li>
                <li>
                  <div class="inner personagens zoe">
                    <a href="/vilasesamo2/jogos/nomedojogo8" title="Nome do Jogo 8" class="">
                      <img src="/portal/images/capaPrograma/vilasesamo2/escola-pra-cachorro.jpg">
                    </a>
                   </div>
                   <a class="nome" href="/vilasesamo2/jogos/nomedojogo8" title="Nome do Jogo 8"><i class="sprite-ico-atividades-p">Nome do Jogo 8</a>
                </li>
              </ul>
            </div>
          </div>
          
        <div class="slider-nav">
          <div class="arrow-left arrow personagem">
            <span title="Back" class="sprite-seta-esquerda personagens" style="display:block;"></span>
          </div>
          <div class="arrow-right arrow personagem">
            <span title="Next" class="sprite-seta-direita personagens" style="display:block;"></span>
          </div>
        </div>
      </div>
    </div>
  </div>