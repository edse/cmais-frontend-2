  ...
  public function executeSignin($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        $u = json_decode(file_get_contents("http://jsonip.appspot.com"));
		    $ip = substr($u->ip,0,8);
        $ips = explode(",",$user->getGuardUser()->getAuthorizedIp());
        $validIp = false;
        foreach($ips as $ip){
          if(trim($ip) == trim($u->ip)){
            $validIp = true;
          }
        }
        if(!$validIp){
          $this->getUser()->signOut();
          header("Location: http://tvcultura.com.br?".$u->ip);
          die();
        }
